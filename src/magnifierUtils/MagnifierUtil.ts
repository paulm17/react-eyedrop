import { MagnifierSize } from "../types"
...
export const getColorFromMousePosition = (
  ...
  const genOffset = (event, magnifier, size: MagnifierSize) => {
    const { clientX, clientY } = event
    const { scrollX, scrollY } = magnifier.ownerDocument.defaultView

    const x = clientX + scrollX - left
    const y = clientY + scrollY - top
  ...
  const offset = genOffset(event, magnifier, magnifierSize)
  const pixels = context && context.getImageData(offset.x, offset.y, 1, 1).data
